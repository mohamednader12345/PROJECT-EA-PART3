...
NUM_GENERATIONS = 50
NUM_RUN = 30
OUTPUT_DIR = os.path.join("de_outputs", "experiment_de2") 
...
def mutation(best_individual, b, c, scale_factor, low, up):
    mutant = best_individual + scale_factor * (b - c)
...
def crossOver(target, mutant, crossover_rate):
    ...
    while True:
        ...
        n = (n + 1) % size
        L += 1
        if random.random() > crossover_rate or L == size:
            break
...
def save_run_summaries(run_histories, output_dir):
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["run_index", "seed", "baseline_objective", "final_best", "improvement_percent", "final_wait", "final_avg_queue", "best_timings"])
        for run in run_histories:
            ...
            writer.writerow([run["run_index"], run["seed"], f"{baseline:.6f}", f"{final_best:.6f}", f"{improvement:.4f}", 
                             f"{run['final_wait']:.2f}", f"{run['final_avg_queue']:.4f}", best_timings])
    return csv_path
...
if __name__ == "__main__":
    ...
    for run_idx, seed_val in enumerate(seeds, start=1):
        ...
        print(f"\-- Run {run_idx} (seed {seed_val}) | DE-EA --")
        ...
        for gen in range(NUM_GENERATIONS):
            ...
            best_in_pop = tools.selBest(population, 1)[0]
            if best_overall is None or best_in_pop.fitness.values[0] < best_overall.fitness.values[0]:
                best_overall = creator.Individual(best_in_pop)
                best_overall.fitness.values = best_in_pop.fitness.values

            for i in range(len(population)):
                b_idx, c_idx = selectedIndices(len(population), i)
                mutant = mutation(best_overall, population[b_idx], population[c_idx], F, MIN_GREEN, MAX_GREEN)
        ...
        final_best_metrics = simulate_traffic(best_overall, traffic_stream)
        ...
        run_histories.append({
            ...
            "final_wait": final_best_metrics["total_wait"],
            "final_avg_queue": final_best_metrics["avg_queue"],
